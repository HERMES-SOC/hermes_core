...
from hermes_instrument import config
from hermes_instrument.util import HERMESUserWarning
from hermes_instrument.util.config import (
    CONFIG_DIR,
    _find_config_files,
    _get_user_configdir,
    _is_writable_dir,
...
def test_copy_default_config_without_overwrite(tmpdir, undo_config_dir_patch, monkeypatch):
    ...
    with pytest.warns(HERMESUserWarning):
        copy_default_config()
