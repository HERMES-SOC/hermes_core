...
The :py:class:`~hermes_core.timedata.TimeData` class supports both `Quantity` and `NDData` objects since one may have advantages
...
Given a current CDF File you can load it into a :py:class:`~hermes_core.timedata.TimeData` by providing a path to the CDF file::
    ...
    >>> from hermes_core.timedata import TimeData
    >>> timedata = TimeData.load("hermes_eea_default_ql_20240406T120621_v0.0.1.cdf") # doctest: +SKIP
...
attributes, please see the :py:func:`~hermes_core.timedata.TimeData.global_attribute_template`
function. 
